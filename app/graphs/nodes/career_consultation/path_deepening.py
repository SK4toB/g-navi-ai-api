...
4. 답변은 **최소 2000자 이상**으로 상세하게 작성해주세요
...
**멘토링 가이드라인:**
1. **사용자 컨텍스트 기반 종합 분석**: 제공된 사용자 답변 전체를 종합적으로 분석하여 경로 선택 이유, 현재 상황, 고민, 목표, 동기를 파악하고 개인의 맞춤형 조언 제공
2. **실제 동료 사례 기반 조언**: 위에 언급된 사내 구성원들의 실제 성장 경험을 바탕으로 구체적인 성장 경로 제시
3. **최신 업계 동향 활용**: 제공된 최신 뉴스 및 기술 트렌드 정보를 활용하여 시장 변화에 맞는 성장 전략 제안
4. **SKAX 내부 리소스 활용**: 사내에서 실제로 활용 가능한 멘토링, 스터디, 프로젝트 기회 안내
5. **회사 비전 연계**: 회사의 최신 기술 트렌드 및 전략 방향과 개인 성장을 연결한 조언 제공
6. **단계별 실행 계획**: 사용자 답변 분석 결과와 최신 트렌드를 바탕으로 다음 3-6개월 내 실천 가능한 액션 아이템 제공
7. **사내 네트워킹**: 도움이 될 수 있는 사내 커뮤니티나 팀 소개
...
5. 답변은 최소 **2000자 이상**으로 상세하게 작성해주세요
...
- **시장 전망**: [뉴스 데이터 기반 {path_name} 분야의 성장 전망과 기회 요인]
- **기술 트렌드**: [최신 뉴스에서 확인된 핵심 기술 발전 방향과 **{merged_user_data.get('name', '고객')}님**의 성장 기회]
- **업계 변화**: [시장 변화에 따른 새로운 역할과 스킬 요구사항]
- **성장 기회**: [뉴스 분석 결과 도출된 **{merged_user_data.get('name', '고객')}님**을 위한 구체적 성장 기회]
